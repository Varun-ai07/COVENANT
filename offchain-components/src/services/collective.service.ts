...
import type { Collective, CollectiveProposal, CollectiveTask, CollectiveMetadata } from "../types.js";
import { loadStore, saveStore } from "../persistence.js";

export class CollectiveService {
  private collectives: Map<string, Collective>;
  private proposals: Map<string, CollectiveProposal>;
  ...
  constructor(config: { identityAddress: Address; identityAbi: any; publicClient: PublicClient }) {
    this.identityAddress = config.identityAddress;
    this.identityAbi = config.identityAbi;
    this.publicClient = config.publicClient;
    this.collectives = new Map(Object.entries(loadStore("collectives", {})));
    this.proposals = new Map(Object.entries(loadStore("proposals", {})));
    this.tasks = new Map(Object.entries(loadStore("collective_tasks", {})));
    this.collectiveCounter = loadStore("collective_counter", 0);
    this.proposalCounter = loadStore("proposal_counter", 0);
    this.taskCounter = loadStore("collective_task_counter", 0);
  }
  ...
  async createCollective(
    ...
    saveStore("collective_counter", this.collectiveCounter);
    ...
    saveStore("collectives", Object.fromEntries(this.collectives));
    return collective;
  }
  ...
  createProposal(
    ...
    saveStore("proposal_counter", this.proposalCounter);
    ...
    saveStore("proposals", Object.fromEntries(this.proposals));
    return proposal;
  }
  ...
  createTask(
    ...
    this.taskCounter++;
    saveStore("collective_task_counter", this.taskCounter);
    ...
    saveStore("collective_tasks", Object.fromEntries(this.tasks));
