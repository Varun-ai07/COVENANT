/**
 * DatabaseChecker - Validates database connectivity and health
 * For criterion type: "database_check"
 */
...
/**
 * Perform database health check via API endpoint
 * This assumes the database health is exposed via an API endpoint
 */
...
/**
 * Score database health check result
 */
function scoreDatabaseCheck(
  result: {
    status: number;
    responseTime: number;
    ok: boolean;
...
export async function check(
  ...
  // Extract database health check details from deliverable
  ...
  if (!url) {
    return {
      ...
      maxScore: MAX_SCORE,
      passed: false,
      details: 'No URL provided for database health check',
      evidence: { url: null }
    };
  }

  console.log(`[DatabaseChecker] Checking database health at: ${url}`);
  ...
  const score = scoreDatabaseCheck(result, containsCheck);
