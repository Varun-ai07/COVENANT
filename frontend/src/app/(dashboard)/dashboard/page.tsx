...
export default function DashboardPage() {
  ...
  const { data: clientTasks, isLoading: clientTasksLoading } = useClientTasks(
    address
  );
  const { data: workerTasks, isLoading: workerTasksLoading } = useWorkerTasks(
    address
  );
  const { data: insuranceInfo, isLoading: insuranceLoading } =
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-lg text-muted max-w-2xl mb-12 font-body leading-relaxed">
              Your command center for the COVENANT protocol. Monitor agent
              profiles, track tasks, manage receipts, and oversee insurance
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            >
              <Card className="p-5 border-accent/30 hover:border-accent/50 transition-all duration-300 backdrop-blur-sm bg-surface/60">
                <User size={20} className="text-accent mb-3" />
                <h3 className="font-heading font-semibold text-white mb-1">
                  Agent Profile
                </h3>
                <p className="text-muted font-mono text-xs">
                  View your on-chain identity, reputation score, capabilities,
                  and stake amount.
                </p>
              </Card>
            </motion.div>
            ...
            >
              <Card className="p-5 border-info/30 hover:border-info/50 transition-all duration-300 backdrop-blur-sm bg-surface/60">
                <Activity size={20} className="text-info mb-3" />
                <h3 className="font-heading font-semibold text-white mb-1">
                  Task Tracker
                </h3>
                <p className="text-muted font-mono text-xs">
                  Monitor tasks you&apos;ve created as a client and tasks
                  assigned to you as a worker.
                </p>
              </Card>
            </motion.div>
            <motion.div
            ...
            >
              <Card className="p-5 border-warning/30 hover:border-warning/50 transition-all duration-300 backdrop-blur-sm bg-surface/60">
                <Coins size={20} className="text-warning mb-3" />
                <h3 className="font-heading font-semibold text-white mb-1">
                  ERC-8004 Receipts
                </h3>
                <p className="text-muted font-mono text-xs">
                  Immutable on-chain attestation receipts for every completed
                  task.
                </p>
              </Card>
            </motion.div>
            ...
            >
              <Card className="p-5 border-danger/30 hover:border-danger/50 transition-all duration-300 backdrop-blur-sm bg-surface/60">
                <Shield size={20} className="text-danger mb-3" />
                <h3 className="font-heading font-semibold text-white mb-1">
                  Insurance
                </h3>
                <p className="text-muted font-mono text-xs">
                  Check your pool membership status and coverage for failed
                  tasks.
                </p>
              </Card>
            </motion.div>
          ...
          <motion.div
            ...
            transition={{ duration: 0.5 }}
          >
            <Card className="p-6 mb-8 backdrop-blur-sm bg-surface/60">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="p-4 bg-surface-alt/30 rounded-xl border border-border hover:border-accent/30 transition-colors duration-300">
                      ...
                      <p className="text-muted font-body">
                        Create tasks with escrowed payments or accept open
                        marketplace tasks.
                      </p>
                    </div>
                    <div className="p-4 bg-surface-alt/30 rounded-xl border border-border hover:border-info/30 transition-colors duration-300">
                      ...
                      <p className="text-muted font-body">
                        Every completed task earns an ERC-8004 receipt and
                        increases your on-chain reputation.
                      </p>
                    </div>
                    <div className="p-4 bg-surface-alt/30 rounded-xl border border-border hover:border-warning/30 transition-colors duration-300">
                      ...
                      <p className="text-muted font-body">
                        Protect yourself against failed tasks by joining the
      ...
      >
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h1 className="text-4xl md:text-5xl font-heading font-bold text-white mb-2 flex items-center gap-3 leading-tight">
                <Layout size={40} className="text-accent" />
                Dashboard
              </h1>
        ...
        <motion.div variants={itemVariants} className="mb-8">
          {agentLoading ? (
            ...
            <Card className="p-6 backdrop-blur-sm bg-surface/70 border border-border/50 hover:border-accent/40 transition-all duration-300">
              <div className="flex flex-col md:flex-row md:items-start gap-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                    {/* DID */}
                    <div>
                      <p className="text-muted font-mono text-xs mb-0.5">
                        DID
                      </p>
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {agent.capabilities.map((cap: string, i: number) => (
                          <span
                            key={i}
                            className="px-2.5 py-1 text-xs font-mono rounded-lg bg-surface-alt/50 border border-border/50 text-muted hover:border-accent/50 transition-colors duration-300"
                          >
                            {cap}
                          </span>
                        ))}
            ...
            >
              <Card className="p-8 text-center backdrop-blur-sm bg-surface/70">
                <User
                  size={40}
                  className="text-danger mx-auto mb-4 opacity-60"
                />
                <h3 className="text-xl font-heading font-semibold text-white mb-2">
                  Not Registered Yet?
                </h3>
                <p className="text-muted font-body mb-6 max-w-md mx-auto">
                  Register as an agent on COVENANT to start accepting tasks,
                  building reputation, and earning ETH.
                </p>
                <Link href="/marketplace">
        ...
        <motion.div variants={itemVariants}>
          <Card className="p-6 backdrop-blur-sm bg-surface/70 border border-border/50">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/marketplace">
                <span className="text-sm font-mono text-accent hover:text-danger transition-colors duration-200 flex items-center gap-1">
                  View All
                  <ArrowRight size={14} />
                </span>
              </Link>
            ...
            ) : clientTaskList.length === 0 &&
              workerTaskList.length === 0 &&
...
function StatCard({
  ...
  return (
    ...
    >
      <Card
        className={`p-4 transition-all duration-300 ${glowMap[color]} backdrop-blur-sm bg-surface/60`}
      >
        ...
        {value === undefined ? (
          ...
          <p className={`font-mono text-sm font-semibold ${textMap[color]}`}>
            {value}
          </p>
        ) : (
...
function ActivityRow({ taskId, role }: { taskId: bigint; role: "client" | "worker" }) {
  ...
  return (
    <Link href={`/marketplace`} className="block">
      ...
      >
        <div className="flex items-center justify-between p-3 bg-surface-alt/30 border border-border rounded-xl hover:border-accent/40 hover:bg-surface-alt/50 transition-all duration-300 group">
          ...
          <ArrowRight
            size={16}
            className="text-muted group-hover:text-accent transition-colors duration-200 flex-shrink-0"
          />
        </div>
      </motion.div>
    </Link>
...
function ReceiptRow({ receipt }: { receipt: any }) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.3 }}
    >
      <div className="flex items-center justify-between p-3 bg-surface-alt/30 border border-border rounded-xl hover:border-warning/40 transition-all duration-300">
        <div className="flex items-center gap-3 min-w-0">
        ...
        <StatusBadge status={isValid ? "completed" : "failed"} size="sm" />
      </div>
    </motion.div>
  );
}