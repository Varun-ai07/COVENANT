...
describe("ReputationVerifier", function () {
  ...
  const MOCK_PC = [7n, 8n];
  // ReputationVerifier uses 4 public signals (not 5 like CapabilityVerifier)
  const MOCK_PUB_SIGNALS = [0n, 0n, 0n, 0n];

  async function deployFixture() {
    const [owner, agent] = await ethers.getSigners();

    const Groth16Verifier = await ethers.getContractFactory("contracts/ReputationVerifier.sol:Groth16Verifier");
    const groth16 = await Groth16Verifier.deploy();

    const ReputationVerifier_CF = await ethers.getContractFactory("ReputationVerifier");
    const verifierContract = await ReputationVerifier_CF.deploy(await groth16.getAddress());
  ...
  describe("Proof Verification", function () {
    it("should reject proof from unauthorized issuer", async function () {
      const { verifierContract, agent } = await loadFixture(deployFixture);
      await expect(
        verifierContract.connect(agent).verifyReputationProof(MOCK_PA, MOCK_PB, MOCK_PC, MOCK_PUB_SIGNALS)
    ...
    it("should return false for invalid/zero proof from authorized issuer", async function () {
      ...
      const result = await verifierContract.connect(agent).verifyReputationProof(
