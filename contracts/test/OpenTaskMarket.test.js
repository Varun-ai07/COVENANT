...
describe("OpenTaskMarket", function () {
  ...
  describe("Submitting Bids", function () {
    ...
    beforeEach(async function () {
      ...
      const deadline = (await time.latest()) + 86400;
      await market.connect(client).postTask(ethers.parseEther("1"), deadline, DESCRIPTION_HASH);
      taskId = 1;
    });
    ...
    it("should allow a bidder to submit a bid", async function () {
      ...
      const [price, estimatedTime, proposal, bidAt, bidder, hasCounter, counterPrice, counterTimeEstimate, counterProposal] = await market.getBid(taskId, bidder1.address);
    ...
    it("should reject zero-priced bid", async function () {
      ...
      ).to.be.revertedWith("Bid price must be positive");
    });
  ...
  describe("Selecting Workers", function () {
    ...
    it("should allow client to select a worker", async function () {
      ...
      await expect(market.connect(client).selectWorker(taskId, bidder1.address))
        ...
        .withArgs(taskId, bidder1.address, ethers.parseEther("0.3"), 3600, PROPOSAL_HASH);

      const task = await market.getTask(taskId);
      expect(task.selectedWorker).to.equal(bidder1.address);
      expect(task.status).to.equal(1); // Selected (Open=0, InProgress=1, Completed=2, Cancelled=3)
    });
    ...
    it("should reject selecting a non-bidder", async function () {
      ...
      await expect(market.connect(client).selectWorker(taskId, bidder3.address))
        .to.be.revertedWith("Selected worker has not bid on this task");
    });
    ...
    it("should reject selecting worker twice", async function () {
      ...
      await expect(market.connect(client).selectWorker(taskId, bidder2.address))
        .to.be.revertedWith("Task is not open for selection");
    });
  });
  ...
  describe("Withdrawing Bids", function () {
    ...
    it("should allow bidder to withdraw bid", async function () {
      ...
      const bid = await market.getBid(taskId, bidder1.address);
      expect(bid.price).to.equal(0);
    });
    ...
    it("should reject withdrawing from non-bidder", async function () {
      ...
      await expect(market.connect(bidder2).withdrawBid(taskId))
        .to.be.revertedWith("You have not bid on this task");
    });
    ...
    it("should reject withdrawing after worker selection", async function () {
      ...
      await market.connect(client).selectWorker(taskId, bidder1.address);
      await expect(market.connect(bidder1).withdrawBid(taskId))
        .to.be.revertedWith("Task is not open for bidding");
    });
  });
  ...
  describe("Completing Tasks", function () {
    ...
    it("should allow worker to complete task", async function () {
      const { market, bidder1 } = fixture;

      await expect(market.connect(bidder1).completeTask(taskId))
        .to.emit(market, "TaskCompleted")
        .withArgs(taskId, bidder1.address, ethers.parseEther("0.3"));

      const task = await market.getTask(taskId);
      expect(task.status).to.equal(2); // Completed (Open=0, InProgress=1, Completed=2, Cancelled=3)
    });

    it("should reject completion from non-worker", async function () {
      const { market, client } = fixture;
      await expect(market.connect(client).completeTask(taskId))
        .to.be.revertedWith("Only selected worker can complete task");
    });

    it("should reject completion before selection", async function () {
      const { market, client, bidder1 } = fixture;
      // Post a new task and submit bid but don't select worker
      const deadline = (await time.latest()) + 86400;
      await market.connect(client).postOpenTask(ethers.parseEther("1"), deadline, DESCRIPTION_HASH);
      const newTaskId = 2;
      await market.connect(bidder1).submitBid(newTaskId, ethers.parseEther("0.3"), 3600, PROPOSAL_HASH);

      // Try to complete without selection - selectedWorker will be 0x0, so bidder1 won't match
      await expect(market.connect(bidder1).completeTask(newTaskId))
        .to.be.revertedWith("Only selected worker can complete task");
    });
  ...
  describe("Cancel Task", function () {
    ...
    it("should allow client to cancel task", async function () {
      ...
      const task = await market.getTask(taskId);
      expect(task.status).to.equal(3); // Cancelled (Open=0, InProgress=1, Completed=2, Cancelled=3)
    });
    ...
    it("should reject cancel after worker selection", async function () {
      ...
      await expect(market.connect(client).cancelTask(taskId))
        .to.be.revertedWith("Only open tasks can be cancelled");
    });
  ...
  describe("Get Bid", function () {
    ...
    it("should return specific bid details", async function () {
      const { market, bidder1 } = fixture;
      const [price, timeEstimate, proposal, bidAt, bidder, hasCounter, counterPrice, counterTimeEstimate, counterProposal] =
