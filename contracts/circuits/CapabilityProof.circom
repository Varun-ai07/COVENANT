...
template CapabilityProof() {
    ...
    signal input modelHash;          // hidden implementation (hash of model/implementation)
    signal input sessionNonce;       // unique per proof

    /* PUBLIC INPUTS */
    signal input capabilityId;       // what capability is claimed (numeric ID)
    signal input agentAddress;       // binds proof to agent
    signal input registeredCommitment; // commitment stored on-chain during registration
...
// Main component makes capabilityId, agentAddress, and registeredCommitment public
