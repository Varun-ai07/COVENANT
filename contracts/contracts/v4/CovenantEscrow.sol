...
/// @title CovenantEscrow V4 - Core trust primitive: lock funds, conditional release
/// @notice ~40K gas for create+fund, 96 bytes per task, batch settlement support
    ...
    PausableUpgradeable
{
    ...
    function failTask(uint256 taskId, bytes32 reason) external override nonReentrant {
        require(
            msg.sender == authorizedArbitration || msg.sender == authorizedSettlement || msg.sender == owner(),
            "unauthorized"
        );
        TaskStorage storage task = _tasks[taskId];
        require(
    ...
    function batchSettle(
        ...
        require(taskIds.length == signatures.length, "length mismatch");
        ...
        for (uint256 i = 0; i < taskIds.length; i++) {
            ...
            require(signer == task.client, "invalid signature");
            ...
            _totalSettled += amounts[i];
        }

        emit BatchSettled(_totalSettled, taskIds.length);
    }
