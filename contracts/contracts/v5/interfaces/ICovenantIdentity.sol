...
interface ICovenantIdentity {
    ...
    function grantCapability(address agent, bytes32 capabilityHash, uint32 expiry, uint128 valueLimit) external;
    function revokeCapability(address agent, bytes32 capabilityHash) external;
    function isRegistered(address agent) external view returns (bool);
    function getAgent(address agent) external view returns (AgentRecord memory);
    function getCapability(address agent, bytes32 capabilityHash) external view returns (CapabilityRecord memory);
